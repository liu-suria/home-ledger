...
export function nextDue(item, base = todayISO()) {
  ...
  if (item.calendar === "lunar") {
    ...
    for (let i=0;i<800;i++) { const candidate = addDays(today,i), p = lunarParts(candidate); if (p.month === targetMonth && p.day === targetDay && p.leap === !!item.lunarLeap) return dateToISO(candidate); }
    return null;
  }
  ...
  if (item.repeat === "weekly" || item.repeat === "interval") {
    ...
    return dateToISO(addDays(start, Math.ceil(passed/step)*step));
  }
  ...
  for (;n<200;n++) { const candidate = monthDate(start,n*step); if (candidate >= today) return dateToISO(candidate); }
  return null;
}
