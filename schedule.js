...
export const dayDiff = (iso, base = todayISO()) => {
...
const monthDate = (start, months) => {
  const day = start.getDate(),
    n = new Date(start.getFullYear(), start.getMonth() + months, 1);
  n.setDate(
...
export const isRecurring = (item) =>
  item.calendar === "lunar" || (item.repeat && item.repeat !== "none");
export function nextOccurrence(item, base = todayISO()) {
  const start = isoToDate(item.targetDate),
    today = isoToDate(base);
  if (!today) return null;
  const stop = isoToDate(item.repeatUntil);
  const allowed = (date) => !stop || date <= stop;
  if (item.calendar === "lunar") {
    const targetMonth = Number(item.lunarMonth),
      targetDay = Number(item.lunarDay);
    if (!targetMonth || !targetDay) return null;
    for (let i = 0; i < 800; i++) {
      const candidate = addDays(today, i),
        p = lunarParts(candidate);
      if (
        p.month === targetMonth &&
        p.day === targetDay &&
        p.leap === !!item.lunarLeap
      )
  ...
  if (item.repeat === "weekly" || item.repeat === "interval") {
    const step =
      item.repeat === "weekly"
        ? 7
        : Math.max(1, Number(item.intervalDays) || 1);
    const passed = Math.max(0, Math.ceil((today - start) / MS_DAY));
    const candidate = addDays(start, Math.ceil(passed / step) * step);
    return allowed(candidate) ? dateToISO(candidate) : null;
  }
  const step =
    item.repeat === "monthly" ? 1 : item.repeat === "quarterly" ? 3 : 12;
  let n = Math.max(
    0,
    Math.floor(
...
export function occurrencesInRange(item, from, until) {
  const end = isoToDate(until),
    result = [];
  if (!end) return result;
  if (!isRecurring(item)) {
    const date = nextOccurrence(item, from);
