...
var esc=function(v){var e=document.createElement('i');e.textContent=v==null?'':String(v);return e.innerHTML};
var pad=function(n){return String(n).padStart(2,'0')};
var iso=function(d){return d.getFullYear()+'-'+pad(d.getMonth()+1)+'-'+pad(d.getDate())};
var today=function(){return iso(new Date())};
var addDays=function(n){var d=new Date();d.setHours(12,0,0,0);d.setDate(d.getDate()+n);return iso(d)};
var dayDiff=function(value){if(!value)return 99999;var a=new Date(value+'T12:00:00'),b=new Date(today()+'T12:00:00');return Math.round((a-b)/86400000)};
var fmt=function(value){return value?new Intl.DateTimeFormat('zh-CN',{month:'numeric',day:'numeric',weekday:'short'}).format(new Date(value+'T12:00:00')):'未设置'};
...
{id:uid(),title:'ChatGPT Plus 续费',type:'subscription',category:'订阅',date:addDays(1),priority:'normal',repeat:'monthly',amount:20,currency:'USD',payment:'Visa',status:'pending',note:'确认账单金额'},
{id:uid(),title:'宝宝疫苗接种预约',type:'baby',category:'宝宝',date:addDays(3),priority:'high',repeat:'none',status:'pending',note:'携带接种本'},
{id:uid(),title:'更换净水器滤芯',type:'maintenance',category:'家庭维护',date:addDays(6),priority:'normal',repeat:'quarterly',status:'pending',note:'PP 棉与活性炭滤芯'},
{id:uid(),title:'iCloud 家庭版续费',type:'subscription',category:'订阅',date:addDays(9),priority:'normal',repeat:'monthly',amount:68,currency:'CNY',payment:'Apple Pay',status:'pending',note:''},
{id:uid(),title:'检查儿童安全座椅固定情况',type:'baby',category:'宝宝',date:addDays(14),priority:'normal',repeat:'monthly',status:'pending',note:'检查 ISOFIX 与肩带'},
{id:uid(),title:'空调深度清洗',type:'maintenance',category:'家庭维护',date:addDays(21),priority:'normal',repeat:'yearly',status:'pending',note:'客厅和卧室两台'},
{id:uid(),title:'宝宝护照到期前办理',type:'document',category:'证件',date:addDays(45),priority:'high',repeat:'none',status:'pending',note:'提前准备照片和材料'},
{id:uid(),title:'电视延保到期',type:'warranty',category:'保修',date:addDays(62),priority:'low',repeat:'none',status:'pending',note:'检查是否需要续保'},
...
function eventCard(x){var n=dayDiff(x.date),cls=x.status==='done'?' done':n<=0?' urgent':n<=3?' soon':'';var money=x.amount!=null&&x.amount!==''?' · '+esc(x.currency||'CNY')+' '+Number(x.amount).toFixed(2):'';return '<article class="event'+cls+'" data-id="'+esc(x.id)+'"><button class="check" data-action="done" aria-label="完成">'+(x.status==='done'?'✓':'')+'</button><div class="event-main"><div class="event-line"><b>'+esc(x.title)+'</b><span class="badge">'+esc(x.category||labels[x.type]||'事项')+'</span></div><div class="meta">'+esc(labels[x.type]||'事项')+' · '+esc(repeats[x.repeat]||x.repeat||'单次')+money+(x.payment?' · '+esc(x.payment):'')+'</div></div><div class="date"><strong>'+dateText(x.date)+'</strong><small>'+fmt(x.date)+'</small></div><button class="more" data-action="menu" aria-label="更多操作">•••</button><div class="row-menu" hidden><button data-action="edit">编辑</button><button data-action="delay">延期 7 天</button><button data-action="delete" class="danger">删除</button></div></article>'}
